...
class HardwareObjectAdapterManager:
    ...
    def get_match_depth(self, ho, supported_types):
        ...
        return min(
            (ho.__class__.mro().index(t) for t in supported_types if t in ho.__class__.mro()),
            default=float("inf")
        )
    ...
    def find_best_adapter(self, ho):
        ...
        for adapter_cls in AdapterBase._SUBCLASSES:
        ...
        for ho_name in _hwr.hardware_objects:
            ...
            if adapter_cls:
                try:
                    ...
                    logging.getLogger("MX3.HWR").info(f"Added adapter for {_id}")
                except Exception:
                    logging.getLogger("MX3.HWR").exception(f"Could not add adapter for {_id}")
                    adapter_cls = AdapterBase
                    adapter_instance = AdapterBase(ho, _id, self.app)

                self._add_adapter(_id, adapter_cls, ho, adapter_instance)
            else:
                logging.getLogger("MX3.HWR").info(f"No adapter for {_id}")
    ...
    def _print_adapter_table(self):
        print(make_table(
            ...
            [[item["id"], item["adapter_cls"], item["ho"]] for item in self.adapter_dict.values()]
        ))
